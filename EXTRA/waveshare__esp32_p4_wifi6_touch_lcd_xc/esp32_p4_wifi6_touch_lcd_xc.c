...
static sd_pwr_ctrl_handle_t bsp_pwr_ctrl_handle = NULL;  // Global LDO power control handle
...
esp_err_t bsp_sdcard_mount(void)
{
    ...
    if (bsp_pwr_ctrl_handle == NULL) {
        sd_pwr_ctrl_ldo_config_t ldo_config = {
            .ldo_chan_id = 4,
        };
        esp_err_t ret = sd_pwr_ctrl_new_on_chip_ldo(&ldo_config, &bsp_pwr_ctrl_handle);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to create a new on-chip LDO power control driver");
            return ret;
        }
    }
    host.pwr_ctrl_handle = bsp_pwr_ctrl_handle;
...
esp_err_t bsp_sdcard_unmount(void)
{
    ...
    if (ret == ESP_OK && bsp_pwr_ctrl_handle != NULL) {
        sd_pwr_ctrl_del_on_chip_ldo(bsp_pwr_ctrl_handle);
        bsp_pwr_ctrl_handle = NULL;
    }
    return ret;
}
